        ...
        for searchTerm in searchTerms:
            ...
            try:
                searchInput = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located((By.ID, "search-sounds"))
                )
            except:
                searchInput = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located((By.ID, "search-input-browse"))
                )
            searchInput.clear()
            searchInput.send_keys(searchTerm)
            searchInput.send_keys(Keys.RETURN)
            lenAudFiles[searchTerm] = {}
            ...
            while not noMorePages:
                ...
                for ele in audioElements:
                    ...
                    audioName = ele.find_element(By.CLASS_NAME, "bw-link--black")
                    lenAudFiles[searchTerm][audioName.text] = duration.text
